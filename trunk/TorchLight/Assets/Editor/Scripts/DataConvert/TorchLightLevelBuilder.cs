...
public class TorchLightLevelBuilder : MonoBehaviour 
{
    ...
    static Dictionary<string, TorchLightLevelLayoutSetConvert.PirceItem>  GAllPieceItems      = null;

    static GameObject GetCachedGameObject(string FbxPath, TorchLightLevelLayoutSetConvert.LevelItem AItem)
    {
        GameObject Obj = null;
    ...
    static Mesh GetCachedCollisionMesh(string ConllisionMeshFile, TorchLightLevelLayoutSetConvert.LevelItem AItem)
    {
    ...
    static public void InstanceLight(TorchLightLevelLayoutSetConvert.LevelItem AItem, GameObject LevelLights)
    {
    ...
    //static public void InstanceConllider<T>(TorchLightLevelLayoutSetConvert.LevelItem AItem, GameObject LevelConllider) where T : Actor
    //{
    //    GameObject ColliderObj = new GameObject(AItem.Name);

    //    T Obj = ColliderObj.AddComponent<T>();
    //    Obj.ExterInfo = AItem.ExternInfo;

    //    ColliderObj.transform.position  = AItem.Position + LightOffset;
    //    ColliderObj.transform.parent    = LevelConllider.transform;
    //}

    static public void InstanceObj(TorchLightLevelLayoutSetConvert.LevelItem AItem, GameObject LevelObjects)
    {
        string MeshFile = "";
        ...
        if (AItem.GUID != null)
        {
            ...
            TorchLightLevelLayoutSetConvert.PirceItem PieceItem = GAllPieceItems[AItem.GUID];
    ...
    static public void InitiAllPieceItems(string ItemsPath)
    {
        if (GAllPieceItems == null)
            GAllPieceItems = TorchLightLevelLayoutSetConvert.GetAllPieceItems();
    }

    static public GameObject LoadLevelLayoutToScene(string LayoutPath)
    {
        InitiAllPieceItems(LayoutPath);

        List<TorchLightLevelLayoutSetConvert.LevelItem> LevelItems = TorchLightLevelLayoutSetConvert.ParseLevelLayout(LayoutPath);
        if (LevelItems != null)
        {
            GameObject Level                = new GameObject(TorchLightTools.GetFileName(LayoutPath));
            ...
            foreach (TorchLightLevelLayoutSetConvert.LevelItem AItem in LevelItems)
            {
                if (AItem.Tag == TorchLightLevelLayoutSetConvert.DESCREPTION_LIGHT)
                {
                    TorchLightLevelBuilder.InstanceLight(AItem, LevelLights);
                }

                if (AItem.Tag == TorchLightLevelLayoutSetConvert.DESCREPTION_MONSTER)
                {
                    TorchLightLevelBuilder.InstanceObj(AItem, LevelNpcs);
                }

                //if (AItem.Tag == TorchLightLevelLayoutSetConvert.DESCREPTION_UNIT_TRIGGER)
                //{
                //    TorchLightLevelBuilder.InstanceConllider<TriggerActor>(AItem, LevelTriggers);
                //}

                //if (AItem.Tag == TorchLightLevelLayoutSetConvert.DESCREPTION_WARPER)
                //{
                //    TorchLightLevelBuilder.InstanceConllider<WarperActor>(AItem, LevelWarpers);
                //}

                if (AItem.Tag == TorchLightLevelLayoutSetConvert.DESCREPTION_ROOM_PIECE)
                {
